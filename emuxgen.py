...
class Generator(object):
	...
	def reset(self):
		return 'cpu_reset_all()'
	def storeBlobArray(self, name, blob):
		return 'uint32_t %s[] = {%s}' % (name, ', '.join('0x%08x' % x for x in blob))
	def loadBlob(self, load, name, blob):
		return 'pc = loadBlob(cpu, 0x%08x, %i, %s)' % (load, len(blob), name)

	def writeGPR(self, gpr, value):
		return 'cpu->R[%i] = %s' % (gpr, value)
	def readGPR(self, gpr):
		return 'cpu->R[%i]' % gpr

	def writeMemory(self, size, ptr, value):
		if size == 8:
			return 'mem_writeb(cpu, %s, %s)' % (value, ptr)
		elif size == 16:
			return 'mem_writew(cpu, %s, %s)' % (value, ptr)
		elif size == 32:
			return 'mem_writel(cpu, %s, %s)' % (value, ptr)
	def readMemory(self, size, ptr):
		if size == 8:
			return 'mem_readb(cpu, %s)' % (ptr)
		elif size == 16:
			return 'mem_readw(cpu, %s)' % (ptr)
		elif size == 32:
			return 'mem_readl(cpu, %s)' % (ptr)
...
run('emuxtemplate.cpp', 'cputest.cpp', Generator())
